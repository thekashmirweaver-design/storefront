...
export function CartDrawer() {
  ...
  const items = cart
    ...
    .filter((i): i is { slug: string; qty: number; product: NonNullable<ReturnType<typeof getProduct>> } => !!i.product);
    ...
    <Sheet open={cartOpen} onOpenChange={setCartOpen}>
      <SheetContent side="right" className="w-full sm:max-w-md bg-background border-l border-border p-0 flex flex-col">
        <SheetHeader className="px-6 py-5 border-b border-border/40">
          <SheetTitle className="font-display text-2xl text-cream text-left">Your Bag ({items.length})</SheetTitle>
        </SheetHeader>
          ...
          <>
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              {items.map((i) => (
                <div key={i.slug} className="flex gap-4">
                  <Link to="/product/$slug" params={{ slug: i.slug }} onClick={() => setCartOpen(false)} className="block w-20 h-24 bg-card overflow-hidden shrink-0">
                    <img src={i.product.image} alt={i.product.name} className="h-full w-full object-cover" />
                  </Link>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-2">
                      <Link to="/product/$slug" params={{ slug: i.slug }} onClick={() => setCartOpen(false)} className="font-display text-base text-cream hover:text-gold">
                        {i.product.name}
                      </Link>
                      <button onClick={() => removeFromCart(i.slug)} className="text-muted-foreground hover:text-gold shrink-0" aria-label="Remove">
                        <X className="h-3.5 w-3.5" />
                      </button>
                    </div>
                    <p className="text-[0.65rem] tracking-[0.2em] uppercase text-muted-foreground mt-1">{i.product.categoryLabel}</p>
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center border border-border">
                        <button onClick={() => setQty(i.slug, i.qty - 1)} className="p-1.5 text-muted-foreground hover:text-gold"><Minus className="h-3 w-3" /></button>
                        <span className="px-3 text-xs text-cream">{i.qty}</span>
                        <button onClick={() => setQty(i.slug, i.qty + 1)} className="p-1.5 text-muted-foreground hover:text-gold"><Plus className="h-3 w-3" /></button>
                      </div>
            ...
            <div className="border-t border-border/40 px-6 py-5 space-y-4">
              <div className="flex justify-between text-sm text-cream">
                <span className="text-[0.7rem] tracking-[0.25em] uppercase text-muted-foreground">Subtotal</span>
                <span className="text-gold">${subtotal}</span>
              </div>
              <p className="text-[0.65rem] text-muted-foreground">Shipping and taxes calculated at checkout.</p>
              <button
                onClick={() => {
                  toast.success("Order placed", { description: "This demo store doesn't process real payments." });
                  clearCart();
                  setCartOpen(false);
                }}
